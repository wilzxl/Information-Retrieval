...
def load_file(file_path):
	...
	if file_path=="/Users/XianlongZhang/Desktop/WG1.txt":
		print "WG1---Perplexity values when get convergence\n"
	else:
		print "\n------------------------------------------------------------------------------\r"
		print "WG2---Perplexity values when get convergence\n"
	for value in perplexity:
		print (str(perplexity.index(value))+ " " + str(value) + "\n")

	SortedPR = OrderedDict(sorted(PAGE_RANK.iteritems(), key=operator.itemgetter(1), reverse=True))
	if file_path=="/Users/XianlongZhang/Desktop/WG1.txt":
		print "\n*****************************\r"
		print "WG1---Document IDs of the top 50 pages sorted by PageRank\n"
	else:
		print "\n*****************************\r"
		print "WG2---Document IDs of the top 50 pages sorted by PageRank\n"
	count = 1
	for page, value in SortedPR.items():
		print (str(count) + ". " + page + " " + str(value) + "\n")
		count =  count + 1
		if count==51:
			break
